...
public:
	...
	i_client_networkable* Networkable() {
		if (this != nullptr) {
			return reinterpret_cast<i_client_networkable*>(reinterpret_cast<std::uintptr_t>(this) + 0x8);
		}
	}
...
public:
	...
	NETVAR("DT_PlantedC4", "m_flDefuseLength", m_flDefuseLength, float)
...
public:
	...
	NETVAR("DT_BasePlayer", "m_vecAbsVelocity", absvelocity, vec3_t)
	...
	std::string weapon2icon(weapon_t* weapon) {
		int id = weapon->item_definition_index();
		switch (id) {
